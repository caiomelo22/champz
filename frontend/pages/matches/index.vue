<template>
  <div class="mx-4">
    ...
    <div v-else>
      ...
      <v-row v-else>
        <v-col v-if="currentGroupIndex == 0" cols="12" md="8" class="fit-screen-div">
          <v-card>
            <GroupToolbar :current-group-index="currentGroupIndex" :group-name="selectedGroup.name"
        ...
        <v-col cols="12" :md="currentGroupIndex ? 12 : 4" class="fit-screen-div"
          :class="currentGroupIndex ? 'champz-knockout' : 'champz-matches'">
          <v-card>
            <GroupToolbar v-if="currentGroupIndex" :current-group-index="currentGroupIndex" :group-name="selectedGroup.name"
              @previous_stage_click="previous_stage_click" @next_stage_click="next_stage_click" />
            <v-simple-table :class="currentGroupIndex ? 'mx-8' : 'mr-0'">
              <template #default>
                ...
                <tbody>
                  <tr v-for="(match, i) in selectedGroup.matches" :key="i">
                    <td class="champz-font text-center" v-if="
      ...
      <div class="text-center">
        <v-btn large class="mx-auto mb-12" light color="primary" v-if="final" :loading="champzFileLoading"
          @click="generate_champz_file()">Generate Champz File</v-btn>
      </div>
      <v-spacer></v-spacer>
